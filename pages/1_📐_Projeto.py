...
st.markdown(
    "Preencha as dimensões da estrutura para gerar automaticamente "
...
def gerar_excel():
    ...
    linha_cabecalho = linha_inicio_materiais + 1
    ...
    for item in dados_atualizados:
        ...
        ws.cell(linha, 1).value = item["Item"]
        ws.cell(linha, 2).value = item["Quantidade"]
        ws.cell(linha, 3).value = "un."
        ws.cell(linha, 4).value = item["Preço Unitário"]
    ...
    ws.cell(
        linha_condicoes,
        1
    ).value = "CONDIÇÕES COMERCIAIS / OBSERVAÇÕES"
    ...
    for i in range(1, 4):
        ...
        ws.cell(
            ...
            "Digite aqui suas condições comerciais "
            "e observações."
        )
...
st.download_button(
    label="📊 Baixar Orçamento Profissional em Excel (.xlsx)",
    data=excel_data,
...
st.caption(
    "O arquivo Excel pode ser editado posteriormente no Microsoft Excel "
    "ou em programas compatíveis com .xlsx."
)
